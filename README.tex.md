...
TODO - Imitation Learning - Training on data (with epoch = 1 and a fixed learning rate on a periodically-retrieved batch of data) in the state-transition form $\{(s_t, s_{t+1})\}_{t=1}^{N-1}$ to learn a control policy. Learning algorithm summary: **(1)** Apply current prediction policy at the state $s_{t}$ to actuate the predicted state $s_{t+1}$, **(2)** wait until the following time-step, **(3)** observe if the state has been changed from $s_{t+1}$ to the actual state $s_{t+1}'$, **(4)** compute and fit/backpropagate the error in the neural net to update the prediction policy, and **(5)** repeat *ad infinitum* (as necessary to operate the smart home). Input is the smart home state (controllable smart home features and relevant environment states like temperature or time), while output is the smart home feature component of the state (as the environment and time are either only controllable in a control-theoretical sense or not controllable by the smart home).
