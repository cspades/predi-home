...
In particular, a neural net trains on the sequence of states $\{s_t\}_{t=1}^{N}$ of the controllable features of the smart home and updates the autonomous smart home controller model to reflect changes/errors in the prediction of the following state $s_{t+1}$ given the previous state $s_t$, i.e. the control decision policy $\pi(s_t) =  s_{t+1}$ of the smart home, with minimal guidance from or interaction with the resident(s) of the smart home. To control the predictive autonomy of the smart home, an unsupervised algorithm (i.e., a dynamically-trained/updated k-means classifier) will differentiate a "checkpoint" subset of the past training data for the imitation learning model in order to group together approximately equivalent trajectories into classes that can be flexibly activated or deactivated in the re-training of the autonomous smart home controller.
...
TODO - Imitation Learning - Training on data (with epoch = 1 and a fixed learning rate on a periodically-retrieved batch of data) in the state-transition form $\{(s_t, s_{t+1})\}_{t=1}^{N-1}$ to learn a control policy. Algorithm summary: 1) Apply current prediction policy at the state $s_{t}$ to actuate the predicted state $s_{t+1}$, 2) wait until the following time-step, 3) observe if the state has been changed from $s_{t+1}$ to the actual state $s_{t+1}'$, 4) compute and fit/backpropagate the error in the neural net to update the prediction policy, and repeat. Input is the entire state (controllable smart home features and time), while output is the entire state except the time (as time is not controllable).
